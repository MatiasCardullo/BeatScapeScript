...
public class GameManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        spaceX = 8; spaceY = 8; roomX = (spaceX / 2) - 0.5f; roomY = (spaceY / 2) - 0.5f;
        maxEnemies = 5; minEnemies = 3;
        CreateScene();
    ...
    /// <summary>
    /// Instance all the objects of the level, also activate/deactivate the stair in first/last level
    /// </summary>
    ...
    /// Create a big room
    /// </summary>
    /// <param name="connectedRoom">char of room</param>
    /// <param name="aux">position</param>
    /// <returns> if its created or not </returns>
    ...
    /// <summary>
    /// Check the adyacent room to see if it needs a wall or a door
    /// </summary>
    /// <param name="dir">char posicion</param>
    /// <param name="coord">coordinate of the room</param>
    /// <returns> char posicion or '_' </returns>
    private char Interaction(char dir,Vector2Int coord)
	{
        char output;
		bool grid, toAdd;
		grid = Grid[level].Exists(aux => aux.xy == coord);
        toAdd = ToAdd.Exists(aux => aux.xy == coord);
		if (grid || toAdd)
        {
			switch (dir)
			{
				case 'e':
					grid = grid && Grid[level].Find(aux => aux.xy == coord).w == true;
					toAdd = toAdd && ToAdd.Find(aux => aux.xy == coord).w == true;
					break;
				case 'n':
					grid = grid && Grid[level].Find(aux => aux.xy == coord).s == true;
					toAdd = toAdd && ToAdd.Find(aux => aux.xy == coord).s == true;
					break;
				case 's':
					grid = grid && Grid[level].Find(aux => aux.xy == coord).n == true;
					toAdd = toAdd && ToAdd.Find(aux => aux.xy == coord).n == true;
					break;
				case 'w':
					grid = grid && Grid[level].Find(aux => aux.xy == coord).e == true;
					toAdd = toAdd && ToAdd.Find(aux => aux.xy == coord).e == true;
					break;
			}
			if (grid || toAdd)
                output = dir;
    ...
    /// Check the adyacent room to see if it needs a wall or a door
    /// </summary>
    /// <param name="input">coordinate of the room</param>
    /// <returns> 'e' or '_' </returns>
    private char EastInteraction(Vector2Int input) => Interaction('e', input + Vector2Int.right);

    /// <summary>
    /// Check the adyacent room to see if it needs a wall or a door
    /// </summary>
    /// <param name="input">coordinate of the room</param>
    /// <returns> 'n' or '_' </returns>
    private char NorthInteraction(Vector2Int input) => Interaction('n', input + Vector2Int.up);

    /// <summary>
    /// Check the adyacent room to see if it needs a wall or a door
    /// </summary>
    /// <param name="input">coordinate of the room</param>
    /// <returns> 's' or '_' </returns>
    private char SouthInteraction(Vector2Int input) => Interaction('s', input + Vector2Int.down);

    /// <summary>
    /// Check the adyacent room to see if it needs a wall or a door
    /// </summary>
    /// <param name="input">coordinate of the room</param>
    /// <returns> 'w' or '_' </returns>
    private char WestInteraction(Vector2Int input) => Interaction('w', input + Vector2Int.left);
    ...
    /// <summary>
    /// Adds enemies, crates and barrels to a room
    /// </summary>
    ...
    private void CellChildSpawner(float maxEnemies, float minEnemies, Vector2 grid, byte level, Transform enemiesParent, Transform cratesParent)
    {
        if (grid != Vector2Int.zero)
        {
            ...
            {
                Enemies[level - 1].AddRange(SpawnEnemies(maxEnemies, minEnemies, grid, level, enemiesParent));
            }
            Crates[level - 1].AddRange(SpawnCratesAndBarrels(grid, level, cratesParent));
        }
    }

    private List<CellChild> SpawnEnemies(float maxEnemies, float minEnemies, Vector2 grid, byte level, Transform enemiesParent)
    {
        List<CellChild> output = new List<CellChild>();
        if (level > 8)
        {
    ...
    private List<CellChild> SpawnCratesAndBarrels(Vector2 grid, byte level, Transform cratesParent)
    {
        List<CellChild> output = new List<CellChild>();
